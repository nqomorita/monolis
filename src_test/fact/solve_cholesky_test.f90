!> 多重フロント法 Cholesky 前進・後退代入のテストモジュール
module mod_monolis_fact_solve_cholesky_test
  use mod_monolis
...
contains
  ...
  subroutine monolis_fact_solve_cholesky_test
    implicit none
    ...
    call monolis_std_global_log_string("monolis_fact_solve_cholesky")
    ...
    call monolis_fact_solve_cholesky(lu, b)

    call monolis_test_check_eq_R("monolis_fact_solve_cholesky_test", b, x_true)

    call monolis_dealloc_R_1d(x_true)
    call monolis_dealloc_R_1d(b)
    call monolis_mat_finalize_LU(lu)
    call monolis_finalize(mat)
  end subroutine monolis_fact_solve_cholesky_test

end module mod_monolis_fact_solve_cholesky_test
