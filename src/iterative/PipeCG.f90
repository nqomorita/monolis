...
contains
  ...
  subroutine monolis_solver_PipeCG(monoPRM, monoCOM, monoMAT, monoPREC)
    ...
    call monolis_alloc_R_1d(L, NPNDOF)
    ...
    !$acc parallel loop present(P, Q, S, Z)
    do i = 1, NPNDOF
      P(i) = 0.0d0
      Q(i) = 0.0d0
      S(i) = 0.0d0
    ...
    do iter = 1, monoPRM%Iarray(monolis_prm_I_max_iter)
      call monolis_inner_product_main_R_no_comm(NNDOF, R, U, CG(1))
      call monolis_inner_product_main_R_no_comm(NNDOF, V, U, CG(2))
      call monolis_inner_product_main_R_no_comm(NNDOF, R, R, CG(3))
      ...
      call monolis_vec_AXPBY_R(NNDOF, beta, Z, 1.0d0, L, Z)
      call monolis_vec_AXPBY_R(NNDOF, beta, Q, 1.0d0, M, Q)
