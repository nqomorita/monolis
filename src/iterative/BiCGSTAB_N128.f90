!> BiCGStab_N128 法モジュール
module mod_monolis_solver_BiCGSTAB_N128
...
contains
  ...
  !> @ingroup solver
  !> BiCGStab 法
  subroutine monolis_solver_BiCGSTAB_N128(monoPRM, monoCOM, monoMAT, monoPREC)
    implicit none
    !> [in,out] パラメータ構造体
    ...
    do iter = 1, monoPRM%Iarray(monolis_prm_I_max_iter)
      call monolis_inner_product_main_R_N128(monoCOM, NNDOF, R, RT, rho, tdotp, tcomm_dotp)
      if(rho == 0.0d0)then
      ...
      call monolis_inner_product_main_R_N128(monoCOM, NNDOF, RT, V, c2, tdotp, tcomm_dotp)
      if(c2 == 0.0d0)then
        call monolis_global_sorted_inner_product_main_R_N128(monoCOM, &
  ...
  end subroutine monolis_solver_BiCGSTAB_N128

end module mod_monolis_solver_BiCGSTAB_N128
