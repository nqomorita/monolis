...
contains
  ...
  subroutine monolis_solver_PipeBiCGSTAB(monoPRM, monoCOM, monoMAT, monoPREC)
    ...
    !$acc enter data create(R(1:NPNDOF), RT(1:NPNDOF), R0(1:NPNDOF), W0(1:NPNDOF), WT(1:NPNDOF), &
    !$acc                   T(1:NPNDOF), PT(1:NPNDOF), S(1:NPNDOF), ST(1:NPNDOF), Z(1:NPNDOF), &
    !$acc                   ZT(1:NPNDOF), Q(1:NPNDOF), QT(1:NPNDOF), Y(1:NPNDOF), V(1:NPNDOF))

    !# 反復初回に係数ゼロで参照される配列をデバイス上で初期化（create 直後は不定値のため）
    !$acc parallel loop present(PT, S, ST, Z, ZT, V)
    do i = 1, NPNDOF
      PT(i) = 0.0d0
      S(i)  = 0.0d0
      ST(i) = 0.0d0
    ...
    if(is_converge)then
      !$acc exit data delete(R, RT, R0, W0, WT, T, PT, S, ST, Z, ZT, Q, QT, Y, V)
...
!$omp & shared(PT, RT, ST, S, W0, Z, WT, ZT, T, V, R, Q, QT, Y) &
!$omp & firstprivate(NNDOF, alpha, beta, omega) &
!$omp & private(i)
!$omp do
!$acc parallel loop present(PT, RT, ST, S, W0, Z, WT, ZT, T, V, R, Q, QT, Y)
      do i = 1, NNDOF
...
!$omp & private(i)
!$omp do
!$acc parallel loop present(X, PT, T)
        do i = 1, NNDOF
...
!$omp parallel default(none) &
!$omp & shared(X, PT, QT, R, Q, Y, RT, WT, ZT, W0, T, V) &
!$omp & firstprivate(NNDOF, alpha, omega1) &
!$omp & private(i)
!$omp do
!$acc parallel loop present(X, PT, QT, R, Q, Y, RT, WT, ZT, W0, T, V)
        do i = 1, NNDOF
