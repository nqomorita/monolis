module monolis_mf_types_module
  ...
  type :: d_lu_only_symbolic_analysis
    ...
    integer(int64), allocatable :: l_col_ptr(:)
    integer, allocatable :: l_row_ind(:)
...
contains
  ...
  subroutine release_symbolic(symbolic)
    ...
    if (allocated(symbolic%parent)) deallocate(symbolic%parent)
    if (allocated(symbolic%first_child)) deallocate(symbolic%first_child)
    if (allocated(symbolic%next_sibling)) deallocate(symbolic%next_sibling)
    if (allocated(symbolic%postorder)) deallocate(symbolic%postorder)
    if (allocated(symbolic%l_col_ptr)) deallocate(symbolic%l_col_ptr)
    if (allocated(symbolic%l_row_ind)) deallocate(symbolic%l_row_ind)
